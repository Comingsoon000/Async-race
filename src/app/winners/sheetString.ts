...
const addSvgIcon = (parent: HTMLElement): void => {
  parent.innerHTML = `<svg version="1.0" xmlns="http://www.w3.org/2000/svg"
  width="32" height="20.15" viewBox="0 0 1280 806" preserveAspectRatio="xMidYMid meet">
...
export const sheetStringRender = (
  parent: HTMLElement,
  id: number,
  color: string,
  name: string,
  ...
  const [winnersSheetString, SheetStringNumber, SheetStringCar, SheetStringName, SheetStringWins, SheetStringTime] =
    createElements([
      ["div", "sheet__string", ""],
      ["div", "sheet__string-number", `${id}`],
      ["div", "sheet__string-car", ""],
      ["div", "sheet__string-name", `${name}`],
      ["div", "sheet__string-wins", `${wins}`],
      ["div", "sheet__string-time", `${time}`],
    ]);
  appendElements(parent, winnersSheetString, [
    SheetStringNumber,
    SheetStringCar,
    SheetStringName,
    SheetStringWins,
  ...
  addSvgIcon(SheetStringCar);
  SheetStringCar.style.fill = `${color}`;
};
